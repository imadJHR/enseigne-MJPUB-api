...
// Endpoint pour la soumission de la commande
app.post('/api/checkout', async (req, res) => {
    const { formData, orderSummary } = req.body;

    if (!formData || !orderSummary) {
        return res.status(400).json({ success: false, message: 'Données de requête invalides' });
    }
    ...
    const emailContent = `
        ...
        <ul style="list-style-type: none; padding: 0;">
            ${items.map(item => `<li>${item.name} (${item.quantity}x) - ${(item.price * item.quantity).toFixed(2)}€</li>`).join('')}
        </ul>
    ...
    try {
        ...
        res.status(200).json({ success: true, message: 'Commande soumise avec succès.' });
    } catch (error) {
        console.error('Erreur lors de l\'envoi de l\'e-mail:', error);
        res.status(500).json({ success: false, message: 'Erreur lors de l\'envoi de la commande.' });
    }
});
...
app.post('/api/configurator-email', async (req, res) => {
    ...
    const emailContent = `
        ...
        <ul style="list-style-type: none; padding: 0;">
            ...
            <li><strong>Style:</strong> ${configuredItem.name.includes('lumineuse') ? 'Lettres lumineuses' : 'Lettres découpées'}</li>
            ...
            <li><strong>Dimensions:</strong> ${configuredItem.details.height} x ${configuredItem.details.estimatedWidth}</li>
            <li><strong>Couleur Texte:</strong> ${configuredItem.details.textColor}</li>
            <li><strong>Couleur Fond:</strong> ${configuredItem.details.backgroundColor}</li>
            ${configuredItem.details.ledColor ? `<li><strong>Couleur LED:</strong> ${configuredItem.details.ledColor}</li>` : ''}
            ${configuredItem.details.intensity ? `<li><strong>Intensité:</strong> ${configuredItem.details.intensity}</li>` : ''}
            ${configuredItem.details.neonEffect ? `<li><strong>Effet Néon:</strong> ${configuredItem.details.neonEffect}</li>` : ''}
            <li><strong>Type de fixation:</strong> ${configuredItem.details.fixationType}</li>
            <li><strong>Options supplémentaires:</strong> ${configuredItem.details.additionalOptions || 'Aucune'}</li>
        </ul>
    `;
    ...
    try {
        ...
        res.status(200).json({ success: true, message: 'Configuration soumise avec succès.' });
    } catch (error) {
        console.error('Erreur lors de l\'envoi de l\'e-mail:', error);
        res.status(500).json({ success: false, message: 'Erreur lors de l\'envoi de la configuration.' });
    }
});
